...
ggplot(logan_pills_per_year) + 
  ...
  labs(x="Year", y="Total pills", title="In Logan County, pill distribution remains steady", subtitle="Total pills shipped to Logan County per year", caption="Source: DEA ARCOS database, via WashingtonPost") + 
...
ggplot(wv_pills_per_year) + 
  ...
  labs(x="Year", y="Total pills", title="Total pills shipped to counties remain steady or gradually increase between 2006 and 2012", subtitle="Total pills per year shipped to Kanawha, Cabell, Mingo, Raleigh, Logan and Wood Counties", caption="Source:DEA ARCOS Database via Washington Post", fill="County") + 
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2012)) + 
  scale_y_continuous(labels=comma) + 
...
md_pop_2012<- county_population(state="MD", key=key) %>%
  filter(year=="2012")%>%
  select(BUYER_COUNTY, year, population)

md_pills <- summarized_county_annual(state="MD", key=key) %>%
  filter(year=="2012")%>%
  select(BUYER_COUNTY, year, DOSAGE_UNIT) 
  

combined_pop_pills <- md_pop_2012 %>%
  inner_join(md_pills, by=("BUYER_COUNTY")) %>%
  select(BUYER_COUNTY, DOSAGE_UNIT, population) 

pills_per_person <- mutate(combined_pop_pills, pills_per_person =  DOSAGE_UNIT / population)%>%
  select(BUYER_COUNTY, pills_per_person)

combined_income_pills <- bind_cols(md_county_median_household_income, pills_per_person) %>%
  select(BUYER_COUNTY, pills_per_person, estimate)

ggplot(combined_income_pills) +
  geom_point(aes(estimate, pills_per_person)) +
  labs(x="Median Income", y="Pills Per Person2", title="Median Income and Pills Per Person Trend is Unclear", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_y_continuous(labels = comma) +
...
pharmacies_annual <- total_pharmacies_state(state="MD", key=key)%>%
  filter(buyer_county=="HARFORD") %>%
  select(buyer_city, total_dosage_unit)

ggplot(pharmacies_annual) + 
  geom_bar(stat="identity", aes(buyer_city, total_dosage_unit), fill="forest green") + 
  labs(x="Town", y="Total pills", title="BEL AIR RECEIVES MOST PILLS BETWEEN 2006 AND 2012", subtitle="Pills shipped to each town in Harford County", caption="Source: DEA ARCOS database, via WashingtonPost") + 
  scale_y_continuous(labels = comma)+
