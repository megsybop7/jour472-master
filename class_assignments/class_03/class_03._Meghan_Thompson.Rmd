...
# The data has 65,359 rows. 
# The drug code, zip code,transaction ID and conversoon factor are each stored as "dbl" but we'd never do math on them. The transaction date is stored as a "chr" but it should be stored differently because it's a date, not a word. 
...
# Buyer zip represents the zip code of the pharmacy who made a purchas. The mean value is 32983 which is not meaningful because zip codes are locations, not quantitative data. 

# Dosage_unit represents how many individual pills were sent to that location. The min is 10, the mean is 697.5 and the max is 25,000. This is meaningful information because it conveys the min, max and average number of pills ordered. The data is quantitative and can be calculated as such. 
...
logan_pipeline<-select(logan, REPORTER_NAME, Combined_Labeler_Name, BUYER_NAME)
view(logan_pipeline)
...
#Built a variable that has everything except for the first four columns. 
...
#Wrote a function that returns all the columns that contain the word buyer 
#view(logan_buyer)
print(logan_buyer)
...
rm(logan_all_but_four, logan_buyer, logan_pipeline, logan_pipline)

#Function that removes everything except for logan. 
...
logan_pipeline_sorted <- logan %>% 
  select(REPORTER_NAME, Combined_Labeler_Name, BUYER_NAME) %>%
...
#I am doing the same thing as before but in descending order.
...
```{r}
select(logan, REPORTER_NAME, Combined_Labeler_Name, BUYER_NAME, DOSAGE_UNIT)%>%
...
select(logan, REPORTER_NAME, Combined_Labeler_Name, BUYER_NAME, DOSAGE_UNIT)%>%
  arrange(desc(DOSAGE_UNIT))%>%
    filter(DOSAGE_UNIT>=15000, REPORTER_NAME== "MCKESSON CORPORATION")%>%
      filter(str_detect(BUYER_NAME, "ARACOMA"))
...
#Which zip code in Logan County the highest single transaction of pills?  

highest_zipcode<-logan%>%
  select(BUYER_ZIP, DOSAGE_UNIT)%>%
  arrange(desc(DOSAGE_UNIT))
...
#What was the average number of pills ordered by pharmacies in the zipcode 25637

average_pills<-logan%>%
  select(BUYER_ZIP, DOSAGE_UNIT)%>%
  filter(BUYER_ZIP=="25637")

summary(average_pills)

#The average pills received in this zipcode was 1135. 

```

```{r}

#Who in this zip code ordered the largest amount of pills in one transaction? 
...
#Family Discount Pharmacy ordered 25,000 pills in one transaction. 
