...
harford_distributors <- total_distributors_county(county= "Harford", state= "MD", key = key)

harford_county_annual <- summarized_county_monthly(county= "Harford", state="MD", key = key)
...
harford_pharmacies <- total_pharmacies_county(county="harford", state="MD", key=key)
...
harford_raw %>%
  group_by(Combined_Labeler_Name, DOSAGE_UNIT)%>%
...
md_pharmacies<- total_pharmacies_state(state="MD", key=key)

md_pharmacies%>%
  group_by(buyer_name, buyer_county, total_dosage_unit)%>%
  summarise(sum(total_dosage_unit))%>%
  arrange(desc(total_dosage_unit))
