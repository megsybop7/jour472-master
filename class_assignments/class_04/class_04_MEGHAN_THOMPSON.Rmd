...
## Zip code is stored as "Dbl" but it should be stored as "chr" because though we would use it to categorize shiptments, we would never do math on the number itself. 
...
baltimore%>%
  group_by(buyer_name, buyer_zip)%>%
  summarise(shipments=n(), total_pills=sum(dosage_unit))%>%
  arrange(desc(total_pills))

##The largest number of pills single location pharmacy was received by Newcare Home Health Services. Owners of the pharmacy were indicted in 2008 for buying and selling hydrocodone online. 

### From 2006 to 2012, Newcare Home Health Services located in East Baltimore received 1,402 shipments of opioids totaling 6,133,600 pills. The DEA and the state of Maryland indicted the owners of the pharmacy in 2008 for selling $10 million worth of hydrocodone online. 
```
...
### The distributor is McKesson Corporation who distributed 58,267,330 pills to the state of Maryland and their headquarters is located in Irving, Texas. 
...
baltimore%>%
  group_by(reporter_name, buyer_name, buyer_zip, buyer_address1)%>%
  summarise(shipment=n())%>%
  arrange(desc(shipment))

###The name of the buyer is Professioanl Pharmacy Services at Sinai Hospital in Baltimore. The number 3,352,600  may be unnusally high because the pharmacy is located in a hospital where demand for painkillers is high. 
```
...
### Rite Aid of Maryland was responsibile for 12 percent of all the pills sent to Baltimore, accounting for 16,740,590 out of 133,721,503 pills. 
...
##There are three different entries for Rite Aid because they each have a different iteration of Rite Aid of Maryland, Inc. One doesn't have a comma and one has T/A at the end. 
```
...
baltimore %>%
  ...
  group_by(buyer_name, rite_aid_y_n)%>%
  summarise(shipments=n(), total_shipments=290908)%>%
  summarise(percent_shipments = (shipments/total_shipments)*100)%>%
  arrange(desc(percent_shipments))
  
## There were 290,908 shipments to Baltimore City and 14.31 percent were sent to Rite Aid. 
