...
mingo_working<-mingo %>%
  mutate(state_distribution= case_when(reporter_state=="WV" ~ "in_state_distribution", TRUE ~ "out_of_state_distribution")
         )
view(mingo_working)
...
#The most significant combo is Activis Pharma, Inc, ---> Miami-Luken ---> Tug Valley Pharmacy who in total received 4,954,200 pills. 
...
mingo %>%
  ...
  group_by(reporter_name, reporter_address1, reporter_zip, reporter_city, reporter_state)
...
### I want to locate each of the variables containing some string and then rename the three variables so they have the same name.
