...
# There are a few records from states including Florida, Indiana, Georgia, Nevada and Ohio that do not have matches to counties. This might create problems because the records are affecting the state data without contributing to general county data. Because of this, we don't know where these records are coming from and how they're affecting the state data without affecting the relative county data. 
```
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map()+
  labs(fill='Pills Per Person', title="West Virginia, Kentucky distributed most pills per person", subtitle="2009 population, U.S. Census", caption="Source: U.S. Census ACS and Washington Post ARCOS Database")+
  theme(legend.position="right")+
  scale_fill_viridis_c(option="magma", labels = comma)
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="From 2009 to 2012, Southeast States Increase Pills Per Person", subtitle = "Pills Per Person Per Year using U.S. Census", caption = "Source: ARCOS database and U.S. Census") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
geo_pills_population_county <- county_geodata_shifted %>%
 inner_join(pills_population_county, by=c("GEOID" = "coxuntyfips"))

mapview(geo_pills_population_county, zcol = "pills_per_person", legend = TRUE)

## West of Appalachia the county pills_per_person is exceptionally high. 
```
...
pills_population_maryland <- pills_population %>%
  ...
  group_by(buyer_county,countyfips, year)%>%
  summarise(total_pills=sum(dosage_unit, na.rm=TRUE),
            total_population=sum(population, na.rm=TRUE))%>%
  mutate(pills_per_person=total_pills/total_population)%>%
  filter(pills_per_person!="Inf")
...
geo_pills_population_maryland %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="From 2006 to 2012, Eastern Shore and Northeastern Maryland increase Pills per Person", subtitle = "Pills Per Person Per Year using U.S. Census", caption = "Source: ARCOS database and U.S. Census") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)

#Over time, Western Maryland, the Eastern Shore and North Eastern Maryland see increases in pills per person distributed in their county. Meanwhile, Southern Marylnad, Baltimore and The D.C. metro area stop increasing after 2011. 
